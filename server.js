...
/* ================== API PARA RELATÓRIO DE TICKETS (FINAL) ================== */
app.get("/api/service-tickets", requireAuth, async (req, res) => {
  try {
    const raw = (req.query.service || "").trim();
    let { start, end, status, customFieldSource, team, classification } = req.query;

    if (customFieldSource && !Array.isArray(customFieldSource)) {
        customFieldSource = [customFieldSource];
    } else if (!customFieldSource) {
        customFieldSource = [];
    }
    const ownerTeam = (team || "").trim();
    ...
    if (ownerTeam) parts.push(`ownerTeam eq '${escapeOData(ownerTeam)}'`);
    if (start) parts.push(`createdDate ge ${start}T00:00:00.000Z`);
    if (end)   parts.push(`createdDate le ${end}T23:59:59.999Z`);
    
    const statusFilter = (() => {
      const s = String(status || "").toUpperCase();
      if (s === "FINALIZADO") return "(baseStatus eq 'Closed' or baseStatus eq 'Resolved')";
      if (s === "CANCELADO") return "(baseStatus eq 'Canceled' or baseStatus eq 'Cancelled')";
      if (s === "PENDENTE") return "(baseStatus ne 'Closed' and baseStatus ne 'Resolved' and baseStatus ne 'Canceled' and baseStatus ne 'Cancelled')";
      return "";
    })();
    if (statusFilter) parts.push(statusFilter);

    if (classification) {
      const [rows] = await db.query(`SELECT customFieldId FROM service_custom_fields_classticket WHERE service = ? LIMIT 1`, [classification]);
      if (rows.length) {
        const classificationFieldId = Number(rows[0].customFieldId);
        parts.push(`customFieldValues/any(c: c/customFieldId eq ${classificationFieldId} and (c/value eq '${escapeOData(classification)}' or c/items/any(i: i/customFieldItem eq '${escapeOData(classification)}')))`);
      }
    }
    ...
    const MOVI_MAX_PAGE_SIZE = 1000;
    
    let allApiData = [];
    let skip = 0;

    while (true) {
        const url = `${MOVI_URL}?token=${MOVI_TOKEN}&$top=${MOVI_MAX_PAGE_SIZE}&$skip=${skip}` +
        ...
        const { data: pageData = [] } = await axios.get(url, { timeout: 60000 });
        
        allApiData.push(...pageData);

        if (pageData.length < MOVI_MAX_PAGE_SIZE) {
            break; 
        }
        
        skip += MOVI_MAX_PAGE_SIZE;
    }
    
    const sourceTableMap = {
        servico: 'service_custom_fields',
        causa: 'service_custom_fields_causa'
    };
    
    const customFieldIdsMap = new Map();
    if (customFieldSource.length > 0) {
        const promises = customFieldSource.map(async (sourceKey) => {
            const tableName = sourceTableMap[sourceKey];
            if (tableName) {
                try {
                    const [rows] = await db.query(`SELECT customFieldId FROM ${tableName} WHERE service = ? LIMIT 1`, [firstLevel]);
                    if (rows.length) customFieldIdsMap.set(sourceKey, rows[0].customFieldId);
                } catch (e) { console.warn(`Falha ao buscar ID para ${sourceKey}:`, e.message); }
            }
        });
        await Promise.all(promises);
    }
    
    const [classificationRows] = await db.query("SELECT customFieldId FROM service_custom_fields_classticket");
    const classificationFieldIds = new Set(classificationRows.map(row => row.customFieldId));
    ...
    const tickets = allApiData.map(t => {
      const owner = t.owner?.businessName || "Não atribuído";
      const solvedDate = t.closedIn || null; 
      
      const extractedValues = {};
      for (const [sourceKey, fieldId] of customFieldIdsMap.entries()) {
          const match = (t.customFieldValues || []).find(cf => Number(cf.customFieldId) === fieldId);
          if (match) {
              if (Array.isArray(match.items) && match.items.length > 0) {
                  extractedValues[sourceKey] = match.items.map(i => i.customFieldItem).filter(Boolean).join(", ");
              } else {
                  extractedValues[sourceKey] = coalesce(match.value, match.customFieldItem, match.valueText, match.text);
              }
          } else {
              extractedValues[sourceKey] = null;
          }
      }

      let ticketClassification = null;
      if (classificationFieldIds.size > 0 && Array.isArray(t.customFieldValues)) {
          const classificationField = t.customFieldValues.find(cf => classificationFieldIds.has(Number(cf.customFieldId)));
          if (classificationField) {
              if (Array.isArray(classificationField.items) && classificationField.items.length > 0) {
                ticketClassification = classificationField.items.map(i => i.customFieldItem).filter(Boolean).join(", ");
              } else {
                ticketClassification = coalesce(classificationField.value, classificationField.customFieldItem, classificationField.valueText, classificationField.text);
              }
          }
      }
      ...
      return {
        ...
        ticketClassification: ticketClassification || null,
        extractedValues
      };
    });
    ...
    console.error("❌ Erro ao buscar tickets por serviço:", err);
    res.status(500).json({ error: "Falha ao consultar tickets" });
  }
});

/* ================== ADMIN: CUSTOM FIELDS POR SERVIÇO ================== */
...
/* ================== ADMIN: CUSTOM FIELDS POR CAUSA ================== */
app.get("/api/custom-fields-causa", requireAdmin, async (req, res) => {
  try {
    const [rows] = await db.query("SELECT * FROM service_custom_fields_causa ORDER BY service");
    res.json(rows);
  } catch (err) {
    console.error("Erro ao listar custom fields por causa:", err);
    res.status(500).json({ error: "Erro ao listar custom fields por causa" });
  }
});

app.post("/api/custom-fields-causa", requireAdmin, async (req, res) => {
  const { service, customFieldId, customFieldRuleId } = req.body;
  if (!service || !customFieldId || !customFieldRuleId) {
    return res.status(400).json({ error: "Campos obrigatórios ausentes" });
  }
  try {
    await db.query(
      `INSERT INTO service_custom_fields_causa (service, customFieldId, customFieldRuleId)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE customFieldId=VALUES(customFieldId), customFieldRuleId=VALUES(customFieldRuleId)`,
      [service, customFieldId, customFieldRuleId]
    );
    ...
    console.error("Erro ao salvar custom field por causa:", err);
    res.status(500).json({ error: "Erro ao salvar custom field por causa" });
  }
});

app.delete("/api/custom-fields-causa/:id", requireAdmin, async (req, res) => {
  try {
    const [r] = await db.query("DELETE FROM service_custom_fields_causa WHERE id = ?", [req.params.id]);
    if (r.affectedRows === 0) return res.status(404).json({ error: "Registro não encontrado" });
    res.json({ success: true });
  } catch (err) {
    console.error("Erro ao excluir custom field por causa:", err);
    res.status(500).json({ error: "Erro ao excluir custom field por causa" });
  }
});

/* ================== ADMIN: CLASSIFICAÇÃO DE TICKET ================== */
app.get("/api/ticket-classifications", requireAdmin, async (req, res) => {
  try {
    const [rows] = await db.query("SELECT * FROM service_custom_fields_classticket ORDER BY service");
    res.json(rows);
  } catch (err) {
    console.error("Erro ao listar classificações:", err);
    res.status(500).json({ error: "Erro ao listar classificações" });
  }
});

app.post("/api/ticket-classifications", requireAdmin, async (req, res) => {
  const { classification, customFieldId, customFieldRuleId } = req.body;
  if (!classification || !customFieldId || !customFieldRuleId) {
    return res.status(400).json({ error: "Campos obrigatórios ausentes" });
  }
  try {
    await db.query(
      `INSERT INTO service_custom_fields_classticket (service, customFieldId, customFieldRuleId)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE customFieldId=VALUES(customFieldId), customFieldRuleId=VALUES(customFieldRuleId)`,
      [classification, customFieldId, customFieldRuleId]
    );
    res.json({ success: true });
  } catch (err) {
...
app.delete("/api/ticket-classifications/:id", requireAdmin, async (req, res) => {
  try {
    const [r] = await db.query("DELETE FROM service_custom_fields_classticket WHERE id = ?", [req.params.id]);
    if (r.affectedRows === 0) return res.status(404).json({ error: "Registro não encontrado" });
    res.json({ success: true });
  } catch (err) {
...
app.post("/api/change-password", async (req, res) => {
  ...
  try {
    ...
    await db.query("UPDATE users SET password_hash = ?, must_change_password = 0 WHERE id = ?", [newHash, req.session.userId]);
...
app.get("/api/ticket/:id", requireAuth, async (req, res) => {
  ...
  try {
    ...
    res.json({
      id: ticket.id, subject, status, owner, ownerTeam, createdDate, requester,
      requesterOrganization, actions
    });
  } catch (err) {
...
app.get("/api/tickets", async (req, res) => {
  try {
    ...
    const tickets = data.map((t) => {
      ...
      return {
        id: t.id, subject: t.subject, urgency: t.urgency || "Não definida", baseStatus: t.baseStatus,
        status: t.status || "Não definido", owner: t.owner?.businessName || "Não atribuído", ownerTeam: t.ownerTeam || "Não definido",
        createdDate: t.createdDate, previsaoSolucao: prevISO, overdue, daysUntilDue, dueCategory,
        canceled: isCanceled(t), serviceFull: t.serviceFull || [],
      };
    });
...
app.get("/api/melhorias", async (req, res) => {
  try {
    ...
    const melhorias = rows.map((row) => ({
      id: row.id, titulo: row.titulo, descricao: row.descricao, autor: row.autor,
      status: row.status || 'Enviada', data: row.created_at ? new Date(row.created_at).toISOString() : null,
    }));
    res.json(melhorias);
  } catch (err) {
